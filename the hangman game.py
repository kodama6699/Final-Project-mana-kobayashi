...
def get_word():
    words = ['cat', 'snake', 'monkey','octopus','yes','ok','safety','coronavirus','python','japan','china','korea',
             'semester','college','rest','vacation']
...
def play_game():
    ...
    while guessed == False and tries > 0:
        ...
        #2 user inpts the full word
        elif len(guess) == len(word):
            if guess == word:
                print('Well done, you have guessed teh word!')
                guessed = True
            else:
                print('Sorry, that was not the word we were looking for')

        #3 user inputs letters where the total number of letters =/= total number of letter in the word.
        else:
        ...
        if status == word:
            ...
            print('You have run outof guessed and you haven\'t guessed the word.')
